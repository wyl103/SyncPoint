...
let sucursalesAcordeonEstado = {}; // Guardar qué sucursales están desplegadas { sucursalId: true/false }
...
async function cargarSucursalesYRutas() {
    ...
    try {
        // Cargar sucursales, rutas y clientes en paralelo
        const [resSuc, resRutas, resCli] = await Promise.all([
            fetch(`${API_BASE}/core/sucursales.php?limit=100`),
            fetch(`${API_BASE}/core/rutas.php?limit=100`),
            fetch(`${API_BASE}/core/clientes.php?limit=5000`)
        ]);
        ...
        if (resultSuc.success) {
            ...
            const rutas = resultRutas.success ? (resultRutas.data || []) : [];
            const clientes = resultCli.success ? (resultCli.data || []) : [];
            ...
            if (sucursales.length === 0) {
                container.innerHTML = `
                    <div class="bg-white border border-gray-200 rounded-2xl p-10 text-center shadow-xs">
                        ...
                        <h3 class="text-lg font-bold text-charcoal">No hay sucursales registradas</h3>
                        <p class="text-xs text-gray-400 mt-1 mb-4">Comienza creando tu primera sucursal operativa.</p>
            ...
            // Agrupar conteo de clientes por sucursal_id y por ruta_id
            const clientesPorSucursal = {};
            const clientesPorRuta = {};

            clientes.forEach(c => {
                if (c.sucursal_id) {
                    clientesPorSucursal[c.sucursal_id] = (clientesPorSucursal[c.sucursal_id] || 0) + 1;
                }
                if (c.ruta_id) {
                    clientesPorRuta[c.ruta_id] = (clientesPorRuta[c.ruta_id] || 0) + 1;
                }
            });
            ...
            sucursales.forEach(suc => {
                // Rutas pertenecientes a esta sucursal
                const rutasSucursal = rutas.filter(r => r.fk_sucursal == suc.id || r.sucursal_id == suc.id);
                const totalClientesSucursal = clientesPorSucursal[suc.id] || 0;
                
                // Estado del acordeón (por defecto primera abierta o según sucursalesAcordeonEstado)
                if (sucursalesAcordeonEstado[suc.id] === undefined) {
                    sucursalesAcordeonEstado[suc.id] = true; // Abierto por defecto
                }
                ...
                if (rutasSucursal.length === 0) {
                    ...
                    rutasSucursal.forEach(r => {
                        ...
                        rutasHtml += `
                            <div class="bg-gray-50/80 border border-gray-200 p-4 rounded-xl shadow-2xs hover:border-primary/50 transition flex flex-col justify-between space-y-3">
                                <div>
                                    ...
                                    <span class="px-2 py-0.5 bg-gray-200/70 text-gray-600 text-[10px] font-bold rounded uppercase tracking-wider inline-block">
                                        ${r.ciudad || 'Sin Ciudad'}
                                    </span>
                                </div>

                                <div class="pt-2 border-t border-gray-200/60 flex items-center justify-between text-xs text-gray-500">
                    ...
                    <div class="bg-white border border-gray-200 rounded-2xl shadow-xs overflow-hidden transition-all">
                        ...
                        <div class="p-5 flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white hover:bg-gray-50/50 transition">
                            <div class="flex items-center gap-3.5">
                                <div class="bg-primary/20 w-12 h-12 rounded-2xl flex items-center justify-center text-charcoal shrink-0 border border-primary/30">
                                ...
                                <div>
                                    ...
                                    <div class="flex items-center gap-3 text-xs text-gray-500 font-medium mt-1">
                        ...
                        <!-- Cuerpo del Acordeón (Rutas Asignadas) -->
...
function cambiarSubTabCliente(subTab) {
    const btnDirectorio = document.getElementById('subtab-btn-directorio');
    const btnSucRutas = document.getElementById('subtab-btn-sucursales-rutas');
    ...
    if (subTab === 'directorio') {
        ...
        if (btnDirectorio) {
            btnDirectorio.className = 'px-4 py-2 rounded-xl font-bold text-xs flex items-center gap-2 transition cursor-pointer bg-primary/20 text-charcoal border border-primary/30 shadow-2xs';
        }
        if (btnSucRutas) {
            btnSucRutas.className = 'px-4 py-2 rounded-xl font-semibold text-xs text-gray-500 hover:text-charcoal hover:bg-gray-100 flex items-center gap-2 transition cursor-pointer border border-transparent';
        }
        ...
        if (btnSucRutas) {
            btnSucRutas.className = 'px-4 py-2 rounded-xl font-bold text-xs flex items-center gap-2 transition cursor-pointer bg-primary/20 text-charcoal border border-primary/30 shadow-2xs';
        }
        if (btnDirectorio) {
            btnDirectorio.className = 'px-4 py-2 rounded-xl font-semibold text-xs text-gray-500 hover:text-charcoal hover:bg-gray-100 flex items-center gap-2 transition cursor-pointer border border-transparent';
        }
